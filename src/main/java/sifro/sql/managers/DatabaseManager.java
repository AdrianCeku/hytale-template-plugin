 ...
 *   <li>{@link #query(int, Object...)}: returns your data as a matrix of {@code [rows][fields]} / {@code Object[][]} with possibly <b>mixed</b> datatypes depending on query</li>
 ...
 * <h2>Threading:</h2>
 * <ul>
 *   <li>Statements are run asynchronously by executors</li>
...
public abstract class DatabaseManager {
     ...
     *
     * <p>Stores raw SQL strings keyed by id. Creates a {@link PreparedStatement} on execution and binds parameters.</p>
     */
     ...
     *
     * <p><b>Important:</b> implementations decide whether the returned connection should be closed by the caller.
     * <ul>
     *   <li>MySQL (Hikari): returned connections must be closed (returned to pool).</li>
     *   <li>SQLite: returned connections are long-lived and must NOT be closed per operation.</li>
     * </ul> </p>
     *
    ...
    /**
     * Called after a DB operation finishes to release/close the connection.
     *
     * <p>For pooled implementations (HikariCP) this should close the connection (returning it to the pool).
     * For long-lived connection implementations (SQLite) this should do nothing.</p>
     *
     * @param connection connection used by the operation
     * @throws SQLException if connection close fails (pooled implementations)
     */
